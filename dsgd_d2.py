...
from gpu_work import Worker, D2_Worker
...
def run(args):
    ...
    for rank in range(args.nodes):
        ...
        worker = D2_Worker(rank, model, train_loader, criterion, optimizer, schedule, args)
            ...
            if ite == 0:
                ...
                for worker in worker_list:
                    for name, param in worker.model.named_parameters():
                        param.data = 2*param.data - worker.model_pre_copy[name]
                        param.grad.data = param.grad.data - worker.grad_pre[name]
                    worker.update_model()
                    worker.copy_model()
            ...
            for worker in worker_list:
                for name, param in worker.model.named_parameters():
                    ...
                    for i in range(args.nodes):
                        ...
                        param.data += worker_list[i].model_copy[name]*wij

    fname = str(args.keep) + 'dsgd_d2' + args.topo + args.datasets + args.model + str(args.nodes)
